class MessageDisplay {
    ...
    displayNewMessage(chatroom, username, message, profilePic) {
        ...
        '<img style="width: 40px; height: 40px; border-radius: 50%; margin-left: 10px;" src="' + profilePic + '"></div><div style="float: left" class="col-md-auto">' + 
        '<div class="row"><b>' + username + '</b></div><div class="row">' + message + '</div></div></div>');    
    }

    displayMessage(chatroom, message) {
        chatroom.append("<p  id='message-color' style='margin-left: 65px;' class='message'>" + message + "</p>")
    }
    ...
    displayImage(chatroom, image, previousImage) {
        if (previousImage == true) {
            chatroom.append("<p  id='message-color' style='margin-left: 65px; margin-top: 7px;' class='message'><img src='" + image + "'></p>")
        }
        else {
            chatroom.append("<p  id='message-color' style='margin-left: 65px;' class='message'><img src='" + image + "'></p>")
        }
    }
